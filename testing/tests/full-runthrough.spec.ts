...
export const createElection = async (page) => {
...
test('full runthrough', async ({ page }) => {
  ...
  await page.fill('input[name="election-title"]', 'Playwright Test Election Updated');
  await page.fill('textarea[name="election-description"]', 'Playwright Test Election Description');
  ...
  await expect(raceDialog.locator('#candidate-name-6')).toHaveValue('Candidate 10');
  ...
  const vote = async (page) => {
    ...
    await expect(page.getByText('Test Mode')).toBeVisible({ timeout: 10000000 });
    ...
    const columnHeadingElements = await columnHeadings.evaluateAll((elements) => elements.map(element => element.textContent));
  ...
  await expect(page.getByText("There's only one vote so far.").first()).toBeVisible({ timeout: 100000000 });
  await page.getByRole('link', { name: 'Ballots' }).click();
  await expect(page.getByText('View Ballots')).toBeVisible({ timeout: 10000000 });
  await page.getByRole('link', { name: 'Voting Page' }).click();
  await vote(page);
  await page.getByRole('button', { name: 'Results' }).click();
  await expect(page.getByText('Candidate 1 Wins!')).toBeVisible({ timeout: 10000000 });
});
