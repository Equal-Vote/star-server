...
function getWinners(summaryData: rankedRobinSummaryData, eligibleCandidates: candidate[]) {
  ...
  const headToHeadWins: totalScore[] = eligibleCandidates
    .map(
      c => ({
        index: c.index,
        score: summaryData.pairwiseMatrix[c.index].filter(entry => entry === 1).length
      } as totalScore)
    )
    .sort((a:totalScore, b:totalScore) => -(a.score-b.score));


  // Return all candidates that tie for top score
  const topScore = headToHeadWins[0]
  const winners = [summaryData.candidates[topScore.index]]
  for (let i = 1; i < headToHeadWins.length; i++) {
    if (headToHeadWins[i].score === topScore.score) {
      winners.push(summaryData.candidates[headToHeadWins[i].index])
    }
  }
  return winners
}
