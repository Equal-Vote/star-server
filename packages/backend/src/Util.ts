...
export async function makeID<Uid>(hasCollision: Function){
  ...
  const idLength = 10; // 22^10 = 26 trillion, we should be set for a while
  let id: Uid;
  let i = 0;
  const maxIter = 10;
  do{
    ...
    id = [...Array(idLength)].map( _ => options.charAt(Math.floor(Math.random()*options.length))).join('') as unknown as Uid;
    i++;
